...
export default function TransaksiDLC() {
    const { id } = useRouter().query;

    const { isLoading, error, data } = useQuery(['dlc_detail'], async () => {
      const {data} = await apiMock.get(`/storeMainPage/dlc/${id}`);
      return data;
  });
  
  const {mutate, isSuccess, isError} = useMutationToast(useMutation(async ({ metode_bayar }: PurchaseData) => {
    //metode_bayar.game_id = parseInt(useRouter().query.id as string, 10);
      apiMock.post(`/secured/user/purchasedlc/${id}`, { metode_bayar }, {
          headers: {
